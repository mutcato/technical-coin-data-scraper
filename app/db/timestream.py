...
class TimeStream:
    ...
    def __init__(self, database=settings.TIMESTREAM_DATABASE_TEST, table=settings.TIMESTREAM_TABLE_TEST):
    ...
    def build_records(self, stream)->List:
        ...
        for item_verbose in stream["Records"]:
            if item_verbose["eventName"] == "INSERT":
                ticker, interval = tuple(item_verbose["dynamodb"]["Keys"]["ticker_interval"]["S"].rsplit("_", 1)) 
                timestamp = int(datetime.strptime(item_verbose["dynamodb"]["Keys"]["time"]["S"], "%Y-%m-%d %H:%M:%S").timestamp())
                item = item_verbose["dynamodb"]["NewImage"]
                for measure_name in measure_names:
                    records.append(
                        {
                            "Dimensions": [{"Name": "ticker", "Value": ticker}, {"Name": "interval", "Value": interval}, {"Name": "exchange", "Value": "binance"}], 
            ...
            for batch in self.chunks(records, self.insertion_limit):
                try:
                    ...
                    if response["ResponseMetadata"]["HTTPStatusCode"] != 200:
                        print(f"Error on insertion. Search this log. Error code: {response['ResponseMetadata']['HTTPStatusCode']}. Error: {response}")
