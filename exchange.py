...
class Binance:
    ...
    def get_close_prices(self, symbol, interval:str, start:int, end:int, currency:str="USDT")->Dict:
        candles = self.client.get_klines(symbol=symbol+currency, interval=interval, startTime=start, endTime=end)
    ...
    def get_all_tickers(self, margin:bool=True, currency:bool=False):
        ...
        if not margin:
            tickers = [snake_case(coin["symbol"], coin["quoteAsset"]) for coin in filtered_coins]
            return {"exchange": "binance", "tickers": tickers}
        else:
            tickers = [snake_case(coin["symbol"], coin["quoteAsset"]) for coin in filtered_coins if "MARGIN" in coin["permissions"]]
            return {"exchange": "binance", "tickers": tickers}